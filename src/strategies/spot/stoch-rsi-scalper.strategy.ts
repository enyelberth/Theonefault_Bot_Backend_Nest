...
@Injectable()
export class StochRsiScalperStrategy implements TradingStrategy<StochRsiScalperStrategyConfig> {
  id: string;
  symbol: string;
  config: StochRsiScalperStrategyConfig;

  private readonly logger = new Logger(StochRsiScalperStrategy.name);
  ...
  async run(): Promise<void> {
    this.validateConfig();
    this.logger.log(`Starting StochRSI scalper on ${this.symbol}`);
    ...
    while (this.isRunning) {
      try {
        ...
        if (!this.inPosition && longSignal) {
          ...
          this.logger.log(
            `BUY ${this.symbol} @ ${currentPrice}. K=${stochK.toFixed(2)} D=${stochD.toFixed(2)} Vol=${volume.toFixed(4)}`,
          );
          ...
          if (hitStop || hitTakeProfit || momentumExit || trendExit || timeoutExit) {
            ...
            this.logger.log(
              `SELL ${this.symbol} @ ${currentPrice}. reason=${hitStop ? 'stop' : hitTakeProfit ? 'take_profit' : momentumExit ? 'momentum_exit' : trendExit ? 'trend_exit' : 'timeout'}`,
            );
            this.resetPosition();
          }
        }
      } catch (error) {
        this.logger.error('Error in StochRSI scalper loop', error instanceof Error ? error.stack : String(error));
  ...
  async stop(): Promise<void> {
    ...
    this.logger.log(`Stopping StochRSI scalper on ${this.symbol}`);
  }
