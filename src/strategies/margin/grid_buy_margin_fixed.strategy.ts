...
export class GridBuyMarginFixedStrategy implements TradingStrategy {
  ...
  private reactivateStoppedLevels(currentPrice: number) {
    try {
      ...
      this.logMessages.logError('Error reactivating stopped levels:', error);
    }
  }
  ...
  private async cancelExistingOrdersInRange() {
    try {
      ...
      const activeBuyOrdersInRange = allOrders.filter(
        (o) =>
          o.side === 'BUY' &&
      ...
      for (const order of activeBuyOrdersInRange) {
        try {
          ...
          this.logMessages.logWarn(
            `Canceled BUY order ID ${order.orderId} price ${order.price}`,
          );
        } catch (err) {
  ...
  private async placeBuyOrders(
    ...
    for (let i = 0; i < this.config.ordersLevels.length; i++) {
      ...
      this.logMessages.logInfo(
        `Level ${i} - openBuyOrders: ${hasBuy}, openSellOrders: ${hasSell}, stoppedLossLevels: ${stopped}, buyPrice: ${buyPrice}, currentPrice: ${currentPrice}`,
      );

      if (hasBuy) continue;
      if (stopped) {
        this.logMessages.logInfo(
          `Skipping level ${i} due to previous stop loss.`,
        );
        continue;
      }
      if (hasSell) {
        this.logMessages.logInfo(
          `Skipping buy order at level ${i} because sell order is still open.`,
        );
        continue;
      }
      ...
      this.logMessages.logInfo(
        `Level ${i} - Placing LIMIT BUY order price ${buyPrice}, currentPrice ${currentPrice}`,
      );
      ...
      this.logMessages.logInfo(
        `Placing LIMIT BUY order level ${i}, price ${buyPrice}, quantity ${quantity}`,
      );
    ...
    for (const [i, order] of this.openBuyOrders.entries()) {
      try {
        ...
        if (statusData.status === 'FILLED') {
          ...
          this.logMessages.logInfo(
            `Placing LIMIT SELL order level ${i} at price ${sellPrice}`,
          );
          this.logMessages.logInfo(
            `Filled BUY at ${filledPrice}, placing SELL at ${sellPrice} for quantity ${quantity}`,
          );
    ...
    for (const [i, order] of this.openSellOrders.entries()) {
      try {
        ...
        if (statusData.status === 'FILLED') {
          ...
          const baseCost =
            orderLevel.price /
            (1 + this.config.profitMargin * this.config.gridCount);
          const quantity = orderLevel.quantity;
          const profit = (filledPrice - baseCost) * quantity;

          this.profitLoss += profit;
          this.logMessages.logInfo(
            `Profit/loss updated by ${profit} at level ${i}, total: ${this.profitLoss}`,
          );
  ...
  private async reinsertSellOrders(
    ...
    for (const i of levels) {
      ...
      this.logMessages.logInfo(
        `Attempting to reinsert LIMIT SELL order level ${i}: sellPrice=${sellPrice}, currentPrice=${currentPrice}`,
      );
  ...
  public async clearAllOrderLevels() {
    ...
    this.logMessages.logInfo(
      'Todos los niveles y órdenes han sido eliminados.',
    );
  }
  ...
  public stopOrderLevel(levelIndex: number) {
    if (levelIndex < 0 || levelIndex >= this.config.ordersLevels.length) {
      this.logMessages.logWarn(
        `Índice inválido para detener nivel: ${levelIndex}`,
      );
      return;
    }
  ...
  async stop(): Promise<void> {
    ...
    this.logMessages.logInfo(`Stopping Grid BUY FIXED on ${this.symbol}`);
  }
}
