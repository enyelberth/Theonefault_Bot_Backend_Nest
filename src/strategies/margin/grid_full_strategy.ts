...
export class GridFullStrategy implements TradingStrategy {
  ...
  private reactivateStoppedLevels(currentPrice: number, side: 'buy' | 'sell') {
    const stoppedLevels =
      side === 'buy' ? this.stoppedLossLevelsBuy : this.stoppedLossLevelsSell;
    const orderLevels =
      side === 'buy'
        ? this.config.ordersLevels_buy
        : this.config.ordersLevels_sell;
    ...
    for (const level of [...stoppedLevels]) {
      ...
      if (
        ...
        (side === 'sell' &&
          currentPrice <= orderLevel.price * (1 - this.config.buySafetyMargin))
      ) {
        this.logMessages.logInfo(
          `Reactivating stopped ${side.toUpperCase()} level ${level} at price threshold.`,
        );
  ...
  private async placeBuyOrders(
    ...
    for (let i = 0; i < this.config.ordersLevels_buy.length; i++) {
      ...
      this.logMessages.logInfo(
        `Placing LIMIT BUY order at level ${i} price ${buyPrice} quantity ${quantity}`,
      );
      try {
        ...
        this.logMessages.logSuccess(
          `LIMIT BUY order created ID: ${normalizedOrder.orderId} at level ${i}`,
        );
      } catch (err) {
        this.logMessages.logError(
  ...
  private async placeSellOrders(
    ...
    for (let i = 0; i < this.config.ordersLevels_sell.length; i++) {
      ...
      this.logMessages.logInfo(
        `Placing LIMIT SELL order at level ${i} price ${sellPrice} quantity ${quantity}`,
      );
      try {
        const order = await this.binanceService.createCrossMarginLimitOrder(
          this.symbol,
    ...
    for (const [i, order] of this.openBuyOrders.entries()) {
      try {
        ...
        if (statusData.status === 'FILLED') {
          this.logMessages.logSuccess(
            `Order BUY level ${i} filled ID: ${order.orderId}`,
          );
          ...
          this.logMessages.logInfo(
            `Placing LIMIT SELL order at level ${i} price ${sellPrice}`,
          );
          try {
            const orderSell =
            ...
            this.logMessages.logSuccess(
              `LIMIT SELL order created ID: ${normalizedSellOrder.orderId} at level ${i}`,
            );
          } catch (err) {
            this.logMessages.logError(
              `Error creating LIMIT SELL order at level ${i}:`,
          ...
          this.logMessages.logWarn(
            `BUY order ID ${order.orderId} stuck, canceling...`,
          );
          try {
    ...
    for (const [i, order] of this.openSellOrders.entries()) {
      try {
        ...
        if (statusData.status === 'FILLED') {
          ...
          this.logMessages.logInfo(
            `Placing LIMIT BUY order at level ${i} price ${buyPrice}`,
          );
          try {
            const orderBuy =
              await this.binanceService.createCrossMarginLimitOrder(
          ...
          this.logMessages.logWarn(
            `SELL order ID ${order.orderId} stuck, canceling...`,
          );
          try {
            await this.binanceService.cancelCrossMarginOrder(
              this.symbol,
  ...
  private async reinsertBuyOrders(
    levels: number[],
    priceFilter: any,
    lotSizeFilter: any,
    currentPrice: number,
    ...
    for (const i of levels) {
      ...
      this.logMessages.logInfo(
        `Reinserting LIMIT BUY order at level ${i} price ${buyPrice} quantity ${quantity}`,
      );
      try {
        const order = await this.binanceService.createCrossMarginLimitOrder(
          this.symbol,
        ...
        this.openBuyOrders.set(i, normalizedOrder);
        this.logMessages.logSuccess(
          `Reinserted LIMIT BUY order created ID: ${normalizedOrder.orderId} at level ${i}`,
        );
      } catch (err) {
        this.logMessages.logError(
          `Error reinserting LIMIT BUY order at level ${i}:`,
          err,
        );
      }
      await this.sleep(250);
  ...
  private async reinsertSellOrders(
    ...
    for (const i of levels) {
      ...
      this.logMessages.logInfo(
        `Reinserting LIMIT SELL order at level ${i} price ${sellPrice} quantity ${quantity}`,
      );
      try {
        const order = await this.binanceService.createCrossMarginLimitOrder(
          this.symbol,
  ...
  private async cancelExistingOrdersInRange() {
    try {
      ...
      if (
        !this.config.ordersLevels_buy?.length &&
        !this.config.ordersLevels_sell?.length
      ) {
        this.logMessages.logWarn(
          'No buy or sell levels defined for cancellation range.',
        );
        return;
      }
      ...
      const activeOrdersInRange = allOrders.filter(
        (o) =>
      ...
      for (const order of activeOrdersInRange) {
        try {
          ...
          this.logMessages.logWarn(
            `Canceled order ID ${order.orderId} price ${order.price}`,
          );
