...
export class BinanceAccountService {
  ...
  async getCrossMarginBalances() {
    ...
    return crossMarginInfo.userAssets.filter(
      (asset: any) =>
        parseFloat(asset.free) > 0 ||
        parseFloat(asset.borrowed) > 0 ||
        parseFloat(asset.netAsset) > 0,
    );
  }
  ...
  async getIsolatedMarginBalances() {
    ...
    return isolatedMarginInfo.filter(
      (asset: any) =>
        (asset.baseAsset &&
          (parseFloat(asset.baseAsset.free) > 0 ||
            parseFloat(asset.baseAsset.locked) > 0)) ||
        (asset.quoteAsset &&
          (parseFloat(asset.quoteAsset.free) > 0 ||
            parseFloat(asset.quoteAsset.locked) > 0)),
    );
  }
  ...
  async getAccountBalanceMarginIsolated() {
    ...
    const response = await this.auth.getSigned(
  ...
  async getCrossMarginPositions() {
    try {
      ...
      throw new Error(
        `Error obteniendo posiciones margin cruzado: ${(error as Error).message}`,
      );
    }
  }
  ...
  async getCrossMarginProfitPercent(symbol: string) {
    const { entryPrice, markPrice, positionAmt } =
      await this.getCrossMarginUnrealizedProfit(symbol);
    if (parseFloat(entryPrice) === 0) return 0;
    const profitPercent =
      ((parseFloat(markPrice) - parseFloat(entryPrice)) /
  ...
  async getCrossMarginPNLSummary() {
    if (!this.auth.canUseMargin()) {
      return {
        ...
        riskLevel: 'BAJO',
        liquidationStatus:
          'Margin desactivado en desarrollo. Activa ENABLE_MARGIN_IN_DEV=true para habilitarlo.',
    ...
    const pnlPercentage =
      totalLiabilityOfBtc > 0 ? (unrealizedPNL / totalLiabilityOfBtc) * 100 : 0;
    ...
    return {
      ...
      assetsSummary: accountInfo.userAssets
        .filter(
          (asset: any) =>
            parseFloat(asset.borrowed) > 0 || parseFloat(asset.netAsset) !== 0,
        )
        .map((asset: any) => ({
          asset: asset.asset,
  ...
  async getConsolidatedBalance() {
    ...
    const marginAccount = this.auth.canUseMargin()
      ? await this.getCrossMarginAccountInfo()
      : null;
    let futuresAccount: any[] | null = null;
    try {
    ...
    spotAccount.balances.forEach((asset: any) => {
      ...
      if (free > 0 || locked > 0) {
        if (!consolidatedByAsset[asset.asset]) {
          consolidatedByAsset[asset.asset] = {
            asset: asset.asset,
            spot: 0,
            margin: 0,
            futures: 0,
    ...
    return {
      ...
      byAsset: Object.values(consolidatedByAsset).filter(
        (a: any) => a.total > 0,
      ),
      summary: {
        totalSpot: Object.values(consolidatedByAsset).reduce(
          (sum: number, a: any) => sum + a.spot,
          0,
        ),
        totalMargin: Object.values(consolidatedByAsset).reduce(
          (sum: number, a: any) => sum + a.margin,
          0,
        ),
        totalFutures: Object.values(consolidatedByAsset).reduce(
          (sum: number, a: any) => sum + a.futures,
          0,
        ),
      },
    };
  ...
  async getAvailableToTrade() {
    ...
    const spotFree = spotAccount.balances.reduce(
      (sum: number, asset: any) => sum + parseFloat(asset.free),
      0,
    );
    ...
    if (marginAccount) {
      marginFree = marginAccount.userAssets.reduce(
        (sum: number, asset: any) => sum + parseFloat(asset.free),
        0,
      );
      marginBorrowed = marginAccount.userAssets.reduce(
        (sum: number, asset: any) => sum + parseFloat(asset.borrowed),
        0,
      );
    }

    const totalBorrowingPower = marginAccount
      ? parseFloat(marginAccount.borrowingUser?.totalAssetOfBtc || 0)
      : 0;
    const usedBorrowingPower = marginAccount
      ? totalBorrowingPower - parseFloat(marginAccount.totalNetAssetOfBtc)
      : 0;

    return {
      spot: {
        available: spotFree.toFixed(8),
        locked: spotAccount.balances
          .reduce(
            (sum: number, asset: any) => sum + parseFloat(asset.locked),
            0,
          )
          .toFixed(8),
      ...
      margin: {
        ...
        available: marginAccount
          ? Math.max(
              0,
              parseFloat(marginAccount.totalAssetOfBtc) -
                parseFloat(marginAccount.totalLiabilityOfBtc),
            ).toFixed(8)
          : '0',
        borrowingPower: totalBorrowingPower.toFixed(8),
        usedBorrowingPower: usedBorrowingPower.toFixed(8),
        marginLevel: marginAccount
          ? parseFloat(marginAccount.marginLevel).toFixed(4)
          : 'N/A',
      ...
      recommendation:
        marginBorrowed > 0
          ? 'Tienes deudas activas en margin. Considera repagar préstamos.'
          : 'Sin deudas de margen.',
    };
  }
  ...
  async getEstimatedTotalValue() {
    ...
    return {
      ...
      valuations,
      summary: {
        spotValue: consolidated.byAsset
          .reduce((sum: number, a: any) => sum + a.spot, 0)
          .toFixed(2),
        marginValue: consolidated.byAsset
          .reduce((sum: number, a: any) => sum + a.margin, 0)
          .toFixed(2),
        futuresValue: consolidated.byAsset
          .reduce((sum: number, a: any) => sum + a.futures, 0)
          .toFixed(2),
      },
    };
  }
  ...
  async getMarginUtilization() {
    const marginEnabled = this.auth.canUseMargin();

    if (!marginEnabled) {
      return {
        marginEnabled: false,
        message:
          'Margin deshabilitado en desarrollo. Activa ENABLE_MARGIN_IN_DEV=true.',
        alerts: {
    ...
    const utilizationRatio =
      totalAssetOfBtc > 0
        ? ((totalLiabilityOfBtc / totalAssetOfBtc) * 100).toFixed(2)
        : '0.00';
    ...
    if (marginLevel > 3) {
      ...
      riskLevel = 'ALTO';
      recommendation =
        'Nivel de margen peligroso. Considera añadir colateral o cerrar posiciones';
    } else {
      riskLevel = 'CRÍTICO';
      recommendation =
        'RIESGO DE LIQUIDACIÓN INMINENTE. Acción inmediata requerida.';
    }
  ...
  async getWalletFundsSummary() {
    ...
    const spotAccount = await this.getAccountInfo();
    const spotTotal = spotAccount.balances.reduce(
      (sum: number, asset: any) =>
        sum + parseFloat(asset.free) + parseFloat(asset.locked),
      0,
    );
    ...
    const marginAccount = this.auth.canUseMargin()
      ? await this.getCrossMarginAccountInfo()
      : null;
    const marginTotal = marginAccount
      ? marginAccount.userAssets.reduce(
          (sum: number, asset: any) => sum + parseFloat(asset.netAsset),
          0,
        )
      : 0;
    const marginAvailable = marginAccount
      ? parseFloat(available.margin.available)
      : 0;
    const marginBorrowed = marginAccount
      ? parseFloat(available.margin.borrowed)
      : 0;

    let futuresTotal = 0;
    try {
      const futuresAccount = await this.getFuturesAccountBalance();
      futuresTotal = Array.isArray(futuresAccount)
        ? futuresAccount.reduce(
            (sum: number, asset: any) => sum + parseFloat(asset.walletBalance),
            0,
          )
        : 0;
    } catch (error) {
    ...
    if (spotLocked > spotTotal * 0.3)
      healthStatus = 'ALERTA: Mucho dinero bloqueado en órdenes';
    if (marginBorrowed > 0 && marginBorrowed > marginAvailable)
      healthStatus = 'CRÍTICA: Deuda > Capital disponible';
    if (marginBorrowed > marginAvailable * 0.8)
      healthStatus = 'CUIDADO: Alto nivel de deuda';

    const topAssets = consolidated.byAsset
      .sort((a: any, b: any) => b.total - a.total)
      .slice(0, 5)
      .map((asset: any) => {
        const valuation = estimatedValue.valuations.find(
          (v: any) => v.asset === asset.asset,
        );
        return {
          asset: asset.asset,
          quantity: asset.total.toFixed(8),
          ...
          percentage:
            (
              (parseFloat(valuation?.valueUSDT || 0) / totalFundsUSDT) *
              100
            ).toFixed(2) + '%',
        };
      });
    ...
    return {
      ...
      totalFunds: {
        ...
        totalLockedUSDT: (
          spotLocked +
          (marginAccount ? parseFloat(marginAccount.totalLiabilityOfBtc) : 0)
        ).toFixed(2),
      },
      byAccount: {
        spot: {
      ...
      recommendations: this.getWalletRecommendations(
        spotLocked,
        spotTotal,
        marginBorrowed,
        marginAvailable,
  ...
  private getWalletRecommendations(
    spotLocked: number,
    spotTotal: number,
    ...
    if (spotLocked > spotTotal * 0.3) {
      recommendations.push(
        'Tienes más del 30% del capital bloqueado en órdenes. Considera cancelar órdenes innecesarias.',
      );
    }

    if (marginBorrowed > marginAvailable * 0.8) {
      recommendations.push(
        'Nivel de deuda muy alto. Considera repagar préstamos o añadir colateral.',
      );
    }

    if (totalUSDT < 100) {
      recommendations.push(
        'Capital muy bajo. Considera depositar más fondos para diversificar estrategias.',
      );
    }

    if (marginBorrowed > 0 && marginAvailable === 0) {
      recommendations.push(
        '⚠️ CRÍTICO: Cero capital disponible en margin. Acción inmediata requerida.',
      );
    }

    if (recommendations.length === 0) {
      recommendations.push(
        'Tu billetera está en buen estado. Continúa monitoreando.',
      );
    }
  ...
  async getAssetPerformanceAnalysis() {
    ...
    const topAssets = assetPerformance
      .sort(
        (a: any, b: any) =>
    ...
    const diversification = {
      ...
      topAssetPercentage:
        topAssets.length > 0 ? topAssets[0].portfolioPercentage : '0.00%',
      top5TotalPercentage:
        topAssets
          .slice(0, 5)
          .reduce(
            (sum: number, a: any) => sum + parseFloat(a.portfolioPercentage),
            0,
          )
          .toFixed(2) + '%',
      concentrationLevel:
        topAssets.length > 0 &&
        parseFloat(topAssets[0].portfolioPercentage) > 50
          ? 'ALTA'
          : 'MEDIA',
    };
    ...
    return {
      ...
      diversification,
      recommendation:
        diversification.concentrationLevel === 'ALTA'
  ...
  async getAdvancedRiskMetrics() {
    ...
    const assetPrices = estimatedValue.valuations.map((v: any) =>
      parseFloat(v.priceUSDT),
    );
    const assetValues = estimatedValue.valuations.map((v: any) =>
      parseFloat(v.valueUSDT),
    );
    ...
    const sortinoRatio =
      downwardVolatility > 0
        ? (expectedReturn - riskFreeRate) / downwardVolatility
        : 0;
    ...
    const btcValuation = estimatedValue.valuations.find(
      (v: any) => v.asset === 'BTC',
    );
    ...
    return {
      ...
      volatilityMetrics: {
        ...
        interpretation:
          volatility > 0.3 ? 'ALTA' : volatility > 0.15 ? 'MEDIA' : 'BAJA',
      },
  ...
  private calculateDownwardVolatility(prices: number[]): number {
    ...
    const downwardVariance =
      prices
        .filter((p) => p < mean)
        .reduce((sum, price) => sum + Math.pow(price - mean, 2), 0) /
      prices.length;
  ...
  private getRiskRecommendations(
    ...
    if (volatility > 0.3) {
      recommendations.push(
        'Volatilidad muy alta. Considera añadir activos menos volátiles.',
      );
    }

    if (concentration > 50) {
      recommendations.push(
        'Concentración extrema. Diversifica hacia otros activos.',
      );
    } else if (concentration > 30) {
      recommendations.push(
        'Concentración alta. Considera rebalancear el portafolio.',
      );
    }

    if (liquidity < 10) {
      recommendations.push(
        '⚠️ Liquidez crítica. Reserva capital para emergencias.',
      );
    ...
    if (sharpeRatio < 0.5) {
      recommendations.push(
        'Retorno/Riesgo desfavorable. Revisa tu estrategia de trading.',
      );
    }

    if (recommendations.length === 0) {
      recommendations.push(
        'Métricas de riesgo saludables. Continúa monitoreando.',
      );
    }
