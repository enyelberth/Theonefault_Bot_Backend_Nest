...
export class AccountController {
  ...
  @ApiResponse({
    status: 200,
    description: 'Saldo de la cuenta obtenido correctamente.',
  })
  @ApiNotFoundResponse({
    description: 'Cuenta no encontrada o sin saldo positivo.',
  })
  ...
  @Get(':id')
  @ApiOperation({
    summary: 'Obtener una cuenta por su ID (incluye saldo y tipo)',
  })
  @ApiParam({
    name: 'id',
    type: Number,
  ...
  @ApiResponse({
    status: 200,
    description: 'Cuenta encontrada correctamente con saldos.',
  })
  @ApiNotFoundResponse({
  ...
  @ApiResponse({
    status: 200,
    description: 'Cuentas del usuario obtenidas correctamente con sus saldos.',
  })
  @ApiNotFoundResponse({
    description:
      'No se encontraron cuentas para el usuario con el ID proporcionado.',
  })
  async findByUserIdWithBalances(
  ...
  @Patch('balance')
  @ApiOperation({
    summary: 'Actualizar saldo de una criptomoneda en una cuenta',
  })
  @ApiResponse({ status: 200, description: 'Saldo actualizado correctamente.' })
  @ApiBadRequestResponse({
    description: 'Datos inválidos para actualizar el saldo.',
  })
  ...
  @ApiOperation({
    summary: 'Jerarquía explícita de cuentas: operativa, técnica y patrimonio',
  })
  @ApiQuery({ name: 'userId', required: false, type: Number })
  async getHierarchySummary(@Query('userId') userId?: string) {
    return this.accountService.getHierarchySummary(
  ...
  @ApiOperation({
    summary: 'Snapshots diarios/semanales de balances para auditoría',
  })
  @ApiQuery({ name: 'accountId', required: false, type: Number })
  async getBalanceAuditSnapshots(@Query('accountId') accountId?: string) {
    return this.accountService.getBalanceAuditSnapshots(
  ...
  @ApiOperation({
    summary:
      'Reconciliación entre journalEntryLine, accountBalance y último sync de exchange',
  })
  @ApiQuery({ name: 'accountId', required: false, type: Number })
  async reconcileAccounting(@Query('accountId') accountId?: string) {
    return this.accountService.reconcileAccountingAndExchange(
      accountId ? Number(accountId) : undefined,
    );
  }
}
