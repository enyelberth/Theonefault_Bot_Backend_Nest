...
export class AnalyticsService {
  ...
  private normalizeRange(from?: string, to?: string) {
    ...
    if (fromDate && toDate && fromDate > toDate) {
      throw new BadRequestException('El rango es invalido: from no puede ser mayor que to.');
    }
  ...
  private getPeriodKey(date: Date, granularity: Granularity): string {
    ...
    const dayNumber = Math.floor((Date.UTC(utcYear, date.getUTCMonth(), date.getUTCDate()) - startOfYear) / 86400000) + 1;
  ...
  private resolveGranularity(value?: string): Granularity {
    ...
    throw new BadRequestException('granularity invalida. Usa day, week o month.');
  }

  private async validateBaseCurrency(baseCurrency?: string): Promise<string> {
    const normalized = (baseCurrency ?? 'USDT').trim().toUpperCase();
    const currency = await this.prisma.currency.findUnique({ where: { code: normalized } });
    if (!currency) {
      throw new BadRequestException(`baseCurrency invalida o inexistente: ${normalized}`);
    }
    return normalized;
  }
  ...
  private async getOrdersForRange(from?: string, to?: string, accountId?: number, symbol?: string, strategy?: string) {
  ...
  async getSummary(accountId?: number, baseCurrency?: string) {
    ...
    const byCurrency = new Map<string, { income: Prisma.Decimal; expense: Prisma.Decimal }>();
    for (const line of lines) {
      const key = line.currencyCode.toUpperCase();
      const current = byCurrency.get(key) ?? { income: new Prisma.Decimal(0), expense: new Prisma.Decimal(0) };
    ...
    return {
      ...
      trading: {
        ...
        winRate: closedCount > 0 ? Number(((wins / closedCount) * 100).toFixed(2)) : 0,
        wins,
        losses,
  ...
  async getPnlTimeSeries(granularityInput?: string, from?: string, to?: string, accountId?: number, symbol?: string, strategy?: string) {
    const granularity = this.resolveGranularity(granularityInput);
    const orders = await this.getOrdersForRange(from, to, accountId, symbol, strategy);
  ...
  async getIncomeExpenseSeries(granularityInput?: string, from?: string, to?: string, accountId?: number) {
    const granularity = this.resolveGranularity(granularityInput);
    const { fromDate, toDate } = this.normalizeRange(from, to);
    ...
    const grouped = new Map<string, { income: Prisma.Decimal; expense: Prisma.Decimal }>();
    for (const line of lines) {
      const period = this.getPeriodKey(line.journalEntry.entryDate, granularity);
  ...
  async getBySymbol(from?: string, to?: string, accountId?: number) {
    ...
    for (const order of orders) {
      const current = grouped.get(order.symbol) ?? { count: 0, pnl: new Prisma.Decimal(0) };
      current.count += 1;
  ...
  async getByStrategy(from?: string, to?: string, accountId?: number) {
    ...
    for (const strategy of strategies) {
      symbolToStrategy.set(strategy.symbol, strategy.strategyType || strategy.type.name || 'unknown');
    }

    const grouped = new Map<string, { count: number; pnl: Prisma.Decimal }>();
    for (const order of orders) {
      const strategyName = symbolToStrategy.get(order.symbol) || 'unknown';
      const current = grouped.get(strategyName) ?? { count: 0, pnl: new Prisma.Decimal(0) };
      current.count += 1;
      current.pnl = current.pnl.plus(this.toDecimal(order.profit_loss));
  ...
  async getRiskMetrics(from?: string, to?: string, accountId?: number, symbol?: string, strategy?: string) {
    const series = await this.getPnlTimeSeries('day', from, to, accountId, symbol, strategy);

    const daily = series.map(item => this.toDecimal(item.realizedPnl).toNumber());
    ...
    const grossLoss = Math.abs(negatives.reduce((acc, value) => acc + value, 0));
    const profitFactor = grossLoss > 0 ? grossProfit / grossLoss : 0;

    const mean = daily.length > 0 ? total / daily.length : 0;
    const variance = daily.length > 0
      ? daily.reduce((acc, value) => acc + (value - mean) ** 2, 0) / daily.length
      : 0;
    ...
    const downsideVariance = downside.length > 0
      ? downside.reduce((acc, value) => acc + (value - mean) ** 2, 0) / downside.length
      : 0;

    const sortino = downsideVariance > 0
  ...
  async materializeDailySnapshots(dateInput?: Date) {
    ...
    const incomeExpenseMap = new Map<string, { income: Prisma.Decimal; expense: Prisma.Decimal }>();
    for (const line of lines) {
      ...
      if (['INGRESO', 'DEBIT', 'DEBITO', 'DÉBITO'].includes(line.entryType.toUpperCase())) {
        current.income = current.income.plus(line.amount);
      } else {
        current.expense = current.expense.plus(line.amount);
      }
    ...
    for (const order of orders) {
      const pnl = this.toDecimal(order.profit_loss);
      const map = order.status === 'CLOSED' ? realizedByAccount : unrealizedByAccount;
      map.set(order.accountId, (map.get(order.accountId) ?? new Prisma.Decimal(0)).plus(pnl));
    }

    for (const balance of balances) {
      const key = `${balance.accountId}:${balance.currencyCode.toUpperCase()}`;
      const ie = incomeExpenseMap.get(key) ?? { income: new Prisma.Decimal(0), expense: new Prisma.Decimal(0) };
      const realized = realizedByAccount.get(balance.accountId) ?? new Prisma.Decimal(0);
      const unrealized = unrealizedByAccount.get(balance.accountId) ?? new Prisma.Decimal(0);
  ...
  async materializeDailyKpis(dateInput?: Date) {
    ...
    const dayStart = new Date(Date.UTC(date.getUTCFullYear(), date.getUTCMonth(), date.getUTCDate(), 0, 0, 0));
    const dayEnd = new Date(Date.UTC(date.getUTCFullYear(), date.getUTCMonth(), date.getUTCDate(), 23, 59, 59, 999));
    const risk = await this.getRiskMetrics(dayStart.toISOString(), dayEnd.toISOString());
    ...
    const wins = dayOrders.filter(order => this.toDecimal(order.profit_loss).gt(0)).length;
  ...
  async runDailyAnalyticsJob() {
    ...
    try {
      ...
      if (recent.length >= 2) {
        ...
        const prevAvg = recent.slice(1).reduce((acc, item) => acc + this.toDecimal(item.maxDrawdown).abs().toNumber(), 0) / (recent.length - 1);

        if (prevAvg > 0 && latest > prevAvg * 2) {
          const adminUser = await this.prisma.user.findFirst({ where: { role: 'ADMIN' } as any });
          if (adminUser) {
      ...
      this.logger.log(`Daily analytics job completado: ${snapshot.records} snapshots, ${kpi.totalTrades} trades analizados.`);
    } catch (error) {
  ...
  async getDashboard(params: {
    ...
    const prismaAny = this.prisma as any;
    const [summary, pnlSeries, incomeExpense, bySymbolAll, byStrategyAll, risk, snapshots] = await Promise.all([
      this.getSummary(accountId, normalizedBase),
      this.getPnlTimeSeries(granularity ?? 'day', from, to, accountId, symbol, strategy),
    ...
    const bySymbol = bySymbolAll.slice(pagination.skip, pagination.skip + pagination.pageSize);
    const byStrategy = byStrategyAll.slice(pagination.skip, pagination.skip + pagination.pageSize);
  ...
  async exportDashboardCsv(params: {
    ...
    for (const item of dashboard.charts.pnlSeries) {
      rows.push(`pnlSeries,${item.period},,${item.realizedPnl},${item.cumulativePnl},`);
    }

    for (const item of dashboard.charts.incomeExpense) {
      rows.push(`incomeExpense,${item.period},,${item.income},${item.expense},${item.net}`);
    }
    ...
    for (const item of dashboard.charts.snapshots) {
      rows.push(`snapshots,${new Date(item.snapshotDate).toISOString()},,${item.net},${item.realizedPnl},${item.unrealizedPnl}`);
    }
