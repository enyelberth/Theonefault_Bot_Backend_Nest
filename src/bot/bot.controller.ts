...
import { OrderLevelDto, StartBotDto, UpdateOrderLevelPriceDto, UpdateProfitMarginDto } from './dto/create-bot.dto';
...
export class BotController {
  ...
  @Get('execution-log')
  @ApiOperation({ summary: 'Bitácora de órdenes y eventos de ejecución en Binance por estrategia' })
  getExecutionLog(
    @Query('symbol') symbol?: string,
    @Query('strategyId') strategyId?: string,
  ) {
  ...
  @Get('performance')
  @ApiOperation({ summary: 'Estadísticas de rendimiento por estrategia (winrate, pnl, fills, etc.)' })
  getPerformance(@Query('strategyId') strategyId?: string) {
    return this.botService.getPerformance(strategyId);
  }

  @Get('performance-history')
  @ApiOperation({ summary: 'Historial de performance por día basado en eventos persistidos' })
  ...
  @Get('dashboard')
  @ApiOperation({ summary: 'Dashboard consolidado de órdenes, fills y PnL por estrategia' })
  async getDashboard(@Query('strategyId') strategyId?: string) {
    return this.botService.getDashboard(strategyId);
  }

  @Patch('risk/:strategyId')
  @ApiOperation({ summary: 'Actualizar controles de riesgo en configuración de estrategia' })
  async updateRisk(
    ...
    return { message: `Controles de riesgo actualizados para estrategia ${strategyId}` };
  }

  @Post('panic-stop/:strategyId')
  @ApiOperation({ summary: 'Detiene estrategia y ejecuta liquidación de emergencia de la posición' })
  async panicStop(
    @Param('strategyId') strategyId: string,
